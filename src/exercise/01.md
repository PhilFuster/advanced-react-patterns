...
If the consumer of a Context has to dispatch multiple state updates, to the
point where it's getting to be unwildly, then at that time you can introduce
this pattern of Context Module Functions Patter. You do this by exposing a
Context Module helper function that'll handle dispatching the multiple state
updates.

Instead you also expose a context module function and do that work there.

Not only do this allow for code re-usability, but also prevents from having to
add the
...
pattern can not only help you reduce duplication, but it also helps improve
